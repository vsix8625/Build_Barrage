...
const char *BARR_version_get_str(void)
{
    ...
    snprintf(
        buf, sizeof(buf), "%d.%d.%d", BARR_VERSION_MAJOR, BARR_VERSION_MINOR, BARR_VERSION_PATCH);
...
bool BARR_init(void)
{
    ...
    switch (res)
    {
        ...
        {
            BARR_errlog(
                "This project was created with a newer version of Build Barrage. Please update!");
        ...
        {
            BARR_warnlog(
                "Project initialized with an older Build barrage version (lock: %d, current: %d)",
                BARR_read_version_code(".barr/init.lock"),
                current_version);
            return true;
        }

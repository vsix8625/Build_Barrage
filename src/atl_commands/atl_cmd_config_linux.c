...
static void atl_remove_path_line(void)
{
    ...
    const char *bin_path = atl_get_bin_path();
    char export_line[ATL_BUF_SIZE_1024];
    snprintf(export_line, sizeof(export_line), "export PATH=\"$PATH:%s\"", bin_path);

    bool skip_next_blank = false;
    ...
    while (fgets(line, sizeof(line), f))
    {
        ...
        if (strstr(trimmed, ATL_CONFIG_PATH_MARKER) || strstr(trimmed, export_line))
        {
            // set the flag for next check and skip marker
            skip_next_blank = true;
            continue;
        }

        if (skip_next_blank && ATL_is_blank(trimmed))
        {
            continue;
        }

        if (!ATL_is_blank(trimmed))
        {
...
atl_i32 atl_config_install(atl_i32 argc, char **argv)
{
    ...
    if (!ATL_isfile(config_file))
    {
        ...
        ATL_file_append(config_file, "atl_root_dir:str=\"%s\";\n", ATL_getcwd());
    }
